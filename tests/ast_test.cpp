  ...
  GIVEN("A binary arithmatic expression 3 * (4 + 5) / (-3 - 1)")
  {
    ...
    WHEN("Compile into Bytecode")
    {
      ...
      THEN("Should produces the expected instruction sets")
      {
        ...
        expected.write(eml::op_add_f64, eml::line_num{0});
        expected.write(eml::op_multiply_f64, eml::line_num{0});
        push_number(expected, 3.);
        expected.write(eml::op_negate_f64, eml::line_num{0});
        push_number(expected, 1.);
        expected.write(eml::op_subtract_f64, eml::line_num{0});
        expected.write(eml::op_divide_f64, eml::line_num{0});
